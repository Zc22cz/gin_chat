...
// 聊天  需要 ：发送者ID ，接受者ID ，消息类型，发送的内容，发送类型
func Chat(writer http.ResponseWriter, request *http.Request) {
	...
	//sendMsg(userId, []byte("欢迎进入即时聊天系统"))
}
...
func JoinGroup(userId uint, comId string) (int, string) {
	...
	utils.DB.Where("owner_id = ? and target_id = ? and type = 2", userId, comId).First(&contact)
